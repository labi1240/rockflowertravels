...
## Next.js Runtime Initialization

When starting a development session, ALWAYS execute the `init` tool from `next-devtools-mcp` first. Ensure `npm run dev` is running so you can leverage `nextjs_index` and `nextjs_call` to read live errors, route structures, and server logs directly from http://localhost:3000/_next/mcp.
